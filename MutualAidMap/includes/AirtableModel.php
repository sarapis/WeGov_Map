...
class AirtableCovidModel
{
	...
	static function getNtaList()
	{
		...
		foreach (self::readTable(COVID_SHEET_NEIB) as $n)
			$rr[$n['fields']['NTACode']] = ['id' => $n['id'], 'code' => $n['fields']['NTACode'], 'name' => $n['fields']['Neighborhood Name'], 'covered' => ($n['fields']['Neighborhood Groups Providing Service is In'] ?? null) <> null];
		return $rr;
	}
